...
x = df['age']
y = df['percent not required of total anaphora']
ax = sns.regplot(x, y, data=df, color='#AD1963')
ax.set(xlabel='Age (in months)', ylabel="% of Anaphoric Utterances")
plt.title('Age vs. Percentage of Total Anaphoric Utterances with Non-Required Extratextual Cues')
plt.show()
...
plt.title('MCDI ProdTotal vs. Percentage of Total Anaphoric Utterances with Non-Required Extratextual Cues')
plt.show()
...
plt.title('MCDI ProdCountNoun vs. Total Anaphoric Utterances with Non-Required Extratextual Cues')
plt.show()