'''
percent_childInst.py

Script to generate plots of percentage of anaphoric 
utterances containing child instigated anaphora as a function of 
...
x = df['age']
y = df['percent child instigated']
ax = sns.regplot(x, y, data=df, color='#AD1963')
ax.set(xlabel='Age (in months)', ylabel="% of Anaphoric Utterances")
plt.title('Age vs. Percentage of Anaphoric Utterances Instigated by Child Subject')
plt.show()
...
plt.title('MCDI ProdTotal vs. Percentage of Anaphoric Utterances Instigated by Child Subject')
plt.show()
...
plt.title('MCDI ProdCountNoun vs. Percentage of Anaphoric Utterances Instigated by Child Subject')
plt.show()